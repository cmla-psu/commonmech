...
def test_mechanism0(choice=1, sigma=1, dims=7):
    """Create Mechanims (B1, S1)."""
    B1, S1 = 0, 0
    if choice == 1:
        B1 = marginal(dims, 2, "one") / (np.sqrt(7)*sigma)
        m = np.shape(B1)[0]
        S1 = np.eye(m)
    if choice == 2:
        B1 = marginal(dims, 2, "two") / (np.sqrt(21)*sigma)
        m = np.shape(B1)[0]
        S1 = np.eye(m)
    if choice == 3:
        n = 2**dims
...
if __name__ == '__main__':
    ...
    #     B2, S2 = test_mechanism(choice=3, sigma=args.sigma, dims=dims)
